...
import { scan, take, finalize } from 'rxjs/operators';
...
export class AppMaterialDesignModule {
    ...
    private progressDialogRef!: MatDialogRef<any>;
    ...
    showProgressDialog(title: string): MatDialogRef<any> {
        this.progressDialogRef = this.matDialog.open(MaterialProgressDialogComponent, {
            width: '380px',
            height: '80px',
        ...
        return this.progressDialogRef;
    }

    public updateProgressDialogTitle(title: string) {
        this.progressDialogRef.componentInstance.data = { dialogTitle: title }
    }

    hideProgressDialog(): void {
        if (undefined != this.progressDialogRef)
